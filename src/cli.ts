...
export async function runCLI(args: string[]): Promise<void> {
  ...
  const sdk: WarbornSDK = createWarbornSDK();
  ...
  switch (command) {
    ...
    case '-v':
    case '--version':
      if (isJson) console.log(JSON.stringify({ cliVersion: '0.1.0', platform: 'Warborn OS' }));
      else console.log('⚡ Warborn CLI v0.1.0 - Building the AI Operating System.');
      break;

    case 'whoami':
      if (isJson) console.log(JSON.stringify({ user: 'developer@warborn.tech', role: 'admin', org: 'Warbornai' }));
      else console.log('👤 Authenticated as: developer@warborn.tech (Role: admin, Org: Warbornai)');
      break;
    ...
    case 'health':
      ...
      if (isJson) console.log(JSON.stringify(doctorResult, null, 2));
      else {
    ...
    case 'agent':
      ...
      if (subCmd === 'list') {
        ...
        if (isJson) console.log(JSON.stringify(agents, null, 2));
        else console.log(`🤖 Active Agents (${agents.length}):`, agents);
      } else {
    ...
    case 'providers':
      ...
      if (isJson) console.log(JSON.stringify(providerList, null, 2));
      else console.log('🧠 Configured Model Providers:', providerList.join(', '));
      break;

    case 'config':
      const configObj = getPlatformConfig();
      if (isJson) console.log(JSON.stringify(configObj, null, 2));
      else console.log('⚙️ Warborn Platform Configuration Mode:', configObj.environment.mode);
      break;
